...
namespace Project.Souccar.Domain.Extensions
{
    public static class TypeExtensions
    {
        ...
        {
            return type.GetInterfaces().Any(inter => inter == typeof (SouccarAggregate)) &&
                   type.GetInterfaces().All(inter => inter != typeof (SouccarIndex));
        }
        public static bool IsAggregateRoot(this Type type)
        {
            return type.GetInterfaces().Any(inter => inter == typeof(SouccarAggregate)) &&
                   type.GetInterfaces().All(inter => inter != typeof(SouccarIndex) && inter != typeof(SouccarConfiguration));
        }
        
        public static bool IsConfigurationRoot(this Type type)
        {
            return type.GetInterfaces().Any(inter => inter == typeof(SouccarConfiguration)) &&
                   type.GetInterfaces().All(inter => inter != typeof(SouccarIndex));
        }
        public static bool IsIndex(this Type type)
        {
            return type.GetInterfaces().Any(inter => inter == typeof(SouccarAggregate)) &&
                   type.GetInterfaces().Any(inter => inter == typeof(SouccarIndex));
        }
